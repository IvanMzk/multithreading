...
)
{
    ...
    auto size_total = std::accumulate(sizes.begin(), sizes.end(), std::size_t{0});
    ...
    for (const auto& size : sizes){
        ...
        std::cout<<std::endl<<size_to_str<value_type>(size)<<" "<<bandwidth_to_str<value_type>(size*iters_per_size, dt_ms_total)<<" copy timer "<<dt_ms_total/iters_per_size;
    }
}